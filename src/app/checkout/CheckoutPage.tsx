    ...
    try {
      ...
      if (selectedItemIds.length === 0 && selectedBookIds.length > 0) {
        ...
        if (successCount === 0) {
          ...
          if (/already\s+borrowed/i.test(rawMsg)) {
            ...
            const outOfStockMsg = await getOutOfStockMessage(firstBookId);
            toast.error(
              outOfStockMsg ??
        ...
        if (typeof window !== "undefined") {
          ...
          window.sessionStorage.removeItem("checkout:selectedItems");
        }
        ...
        router.push(`/success?returnBy=${encodeURIComponent(returnDateLabel)}`);
        return;
      }
      ...
      try {
        ...
        if (!allExistInCart && selectedBookIds.length > 0) {
          ...
          if (uniqueMapped.length > 0) effectiveItemIds = uniqueMapped;
        }
      } catch {
      ...
      effectiveItemIds = Array.from(
        new Set(
          effectiveItemIds
            .map((v) => (Number.isFinite(v) ? Math.trunc(v) : NaN))
            .filter(
      ...
      const borrowRes: BorrowFromCartEnvelope | null = res.data ?? null;
      ...
      const noBooksMessage =
        ...
        borrowRes.message.toLowerCase().includes("no books borrowed");
      ...
      if (loans.length === 0 && noBooksMessage) {
        ...
        await Promise.allSettled(
          effectiveItemIds.map((itemId) => removeCartItemApi({ itemId })),
        );
      ...
      if (borrowedViaFallback) {
        ...
        try {
          const cart = await getMyCartApi();
          const rawItems = Array.isArray(cart.items) ? cart.items : [];
          const remainingIds = new Set(extractCartItemIds(rawItems));
      ...
      await queryClient.invalidateQueries({ queryKey: ["meLoans"] });
      ...
      <div className="hidden md:block px-6xl">
        <div className="mx-auto w-full max-w-300">
          ...
          <main className="pb-6xl">
            <h1 className="pt-6xl text-display-sm font-bold tracking-[-0.02em] text-neutral-950">
              Checkout
            </h1>

            <div className="mt-4xl grid grid-cols-[minmax(0,1fr)_26rem] gap-6xl">
              <section className="min-w-0">
                ...
                <div className="mt-2xl h-px w-full bg-neutral-200" />
              ...
              <aside className="self-start">
                <div className="rounded-3xl border border-neutral-200 bg-base-white p-4xl">
                  <div className="text-text-sm font-bold tracking-[-0.02em] text-neutral-950">
                    Complete Your Borrow Request
                  </div>
                  ...
                  <div className="mt-2xl">
                    ...
                    <div className="relative mt-sm">
                      <input
                        ...
                        value={borrowDate}
                        onChange={(e) => setBorrowDate(e.target.value)}
                        className="h-11 w-full rounded-xl border border-neutral-200 bg-base-white px-lg pr-11 text-text-sm font-semibold tracking-[-0.02em] text-neutral-950"
                        aria-label="Borrow Date"
                      />
                      <CalendarIcon
                        aria-hidden
                        className="pointer-events-none absolute right-lg top-1/2 h-5 w-5 -translate-y-1/2 text-neutral-500"
                      />
                    </div>
                    <div className="sr-only">{borrowDateLabel}</div>
                  </div>
                  ...
                  <button
                    ...
                    disabled={!canConfirm}
                    onClick={handleConfirmBorrow}
                    className="mt-3xl h-11 w-full rounded-full bg-primary-600 px-4xl text-text-sm font-bold tracking-[-0.02em] text-base-white disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {confirmPending ? "Processing..." : "Confirm & Borrow"}
                  </button>
                </div>
